...
const { loadBrowserScripts } = require("./helpers/script-context");

function createSummaryContext() {
  return loadBrowserScripts([
    "components/js/tasks.js",
    "components/js/summary.js",
  ]);
}
...
test("ignores completed, past and invalid deadlines", () => {
  ...
  assert.equal(context.getUpcomingDeadlineText(tasks, today), "July 18, 2026",);
});
...
test("shows a fallback when no upcoming deadline exists", () => {
  ...
  assert.equal(context.getUpcomingDeadlineText(tasks, today), "No upcoming deadline",);
});
