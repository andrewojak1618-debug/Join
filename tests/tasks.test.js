...
const {
  ...
  loadBrowserScripts,
  toPlainValue,
} = require("./helpers/script-context");
...
test("updates only the task with the matching id", () => {
  ...
  assert.deepEqual(toPlainValue(context.getStoredTasks()), [firstTask, { ...secondTask, status: "done" },]);
});
...
test("uses the Firebase adapter when it is available", async () => {
  const calls = [];
  const firebaseTasks = {
    async updateTask(taskId, task) { calls.push({ taskId, task }); },
  };
  const localStorage = createMemoryStorage();
  const context = loadBrowserScripts([TASKS_SCRIPT, TASK_STORE_SCRIPT], { localStorage, window: { joinFirebaseTasks: firebaseTasks }, });
